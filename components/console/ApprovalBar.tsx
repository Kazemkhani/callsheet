 ...
 * so it holds the floor: full width, on ink so it reads as the foot of the
 * charcoal anchor, and the only signal-coloured element on screen while it is
 * up.
 */
export function ApprovalBar({
  ...
  return (
    <div className="sticky bottom-0 z-20 border-t border-ink-rule bg-ink text-paper">
      <div className="mx-auto flex w-full max-w-[1360px] flex-wrap items-center justify-between gap-x-8 gap-y-4 px-8 py-5">
        <p className="min-w-0 text-body">
          ...
          <span className="text-ink-label">Nothing has been sent.</span>
        </p>
        <button
          type="button"
          onClick={onApprove}
